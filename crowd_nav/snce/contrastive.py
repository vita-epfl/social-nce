...
class SocialNCE():
    ...
    def loss(self, robot, human, pos, neg, feat):
        ...
        if self.sampling == 'social':
            sample_pos, sample_neg, mask_valid = self.sampler.social_sampling(robot, human, pos[:,self.horizon-1], neg[:,self.horizon-1])
        elif self.sampling == 'local':
            sample_pos, sample_neg, mask_valid = self.sampler.local_sampling(robot, human, pos[:,self.horizon-1], neg[:,self.horizon-1])
        elif self.sampling == 'event':
            sample_pos, sample_neg, mask_valid = self.sampler.event_sampling(robot, human, pos, neg)
        elif self.sampling == 'positive':
            sample_pos, sample_neg, mask_valid, mask_pos = self.sampler.positive_sampling(robot, human, pos[:,self.horizon-1], neg[:,self.horizon-1])
        else:
        ...
        # obsv embedding
        emb_obsv = self.head_projection(feat[mask_valid])    # a tensor of shape (B, E) for motion embeddings
        ...
        for i in range(5):
            if len(pos.shape) > 2:
                for t in range(self.horizon):
                    plot_samples(robot[i,:4], human[i], robot[i,4:], pos[i, t], neg[i, t], fname='samples_{:d}_time_{:d}.png'.format(i, t))
            else:
                for t in range(self.horizon):
