...
def main():
    ...
    logname = [fname.split("/")[-2] for fname in args.log_files]
    models = logname
    # models = ['S-LSTM-Timeout-SGD-0.01', 'S-LSTM-Timeout-SGD-0.1', 'S-LSTM-Timeout-SGD-0.1-Target', 'S-LSTM-Timeout-SGD-0.01-Target']
    max_episodes = 10000
    ...
    for i, log_file in enumerate(args.log_files):
        ...
        val_pattern = r"VAL   in episode (?P<episode>\d+)  success: (?P<sr>[0-1].\d+), " \
                      r"collision: (?P<cr>[0-1].\d+), nav time: (?P<time>\d+.\d+), " \
        ...
        train_pattern = r"TRAIN in episode (?P<episode>\d+)  success: (?P<sr>[0-1].\d+), " \
                        r"collision: (?P<cr>[0-1].\d+), nav time: (?P<time>\d+.\d+), " \
                        r"reward: (?P<reward>[-+]?\d+.\d+)"
        train_episode = []
        train_sr = []
        ...
        print("val_episode", val_episode[-1])
        print("train_episode", train_episode[-1])
